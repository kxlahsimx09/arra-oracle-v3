...
# ── singleton guard ────────────────────────────────────────────────────────
# Identify other running supervisor instances of this exact script.
# Match key is "<parent-dir-basename>/<script-basename>" (e.g.
# "brew-ops-bot/bot.sh") so we never cross-kill orchestrator-bot.
# Children of the current process (curl getUpdates, subshells) are excluded
# via the PPID guard — they share the script name but are not supervisors.
find_other_daemons() {
  local key
