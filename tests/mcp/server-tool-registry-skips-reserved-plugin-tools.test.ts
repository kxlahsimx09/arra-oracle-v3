...
test('MCP server registry keeps built-in tools ahead of reserved plugin names', async () => {
  ...
  try {
    (server as any).unifiedRuntimeReady = Promise.resolve({ routes: [], menu: [], cliSubcommands: [], servers: [], stop: async () => {}, mcpTools: [{ plugin: 'demo', name: 'oracle_search', handler: 'run', inputSchema: { type: 'object' } }], callMcpTool: async () => 'ok' });
